  ...
  const updateModelValue = useCallback(
    (path: ModelPath, definition: FieldDefinition, value: PrimitiveValues) => {
      setModel((prev) => {
        ...
        // Jika nilai baru berbeda dari nilai lama, hanya update dan trigger render jika berbeda
        if (newModel[path] !== value) {
